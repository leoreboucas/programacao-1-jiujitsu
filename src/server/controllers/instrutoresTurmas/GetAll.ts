...
export const getAllValidation = validation((getSchema) => ({
  query: getSchema<IQueryProps>(yup.object().shape({
    ...
    id_instrutor: yup.number().integer().optional().default(0),
...
export const getAll = async (req: Request<unknown, unknown, unknown, IQueryProps>, res: Response) => {
  const result = await instrutoresTurmas.Provider.getAll(
    Number(req.query.page) || 1,
    Number(req.query.limit) || 10,
    Number(req.query.id) || 0,
    Number(req.query.id_instrutor) || 0,
    Number(req.query.id_turma) || 0
  );
  const count = await instrutoresTurmas.Provider.count(String(req.query.id || ''));
